...
# Additional packages - Rubygems, bundler
RUN gem update --system && \
    gem install bundler && \
    bundle -v

# Copy over Gemfile first so that changes to other app files don't
# trigger re-running of bundle install 
WORKDIR $GEM_HOME
...
CMD ["config/docker/website-startup.sh"]
